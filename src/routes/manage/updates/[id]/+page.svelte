<script lang="ts">
	...
	import ManageUpdateEditor from '$lib/features/manage/components/ManageUpdateEditor.svelte'
	...
	let { data } = $props()
	let record = $state<ManageUpdateDocument | null>(null)
	...
	async function loadRecord(id: string) {
		...
		errorMessage = ''
		try {
			record = await fetchManagedRecord(fetch, 'updates', id)
		} catch (cause) {
			record = null
			errorMessage =
...
{:else if record}
	<ManageUpdateEditor csrfToken={data.session.csrfToken} initialRecord={record} mode="edit" />
{/if}
