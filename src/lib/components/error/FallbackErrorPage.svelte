<script lang="ts">
	...
	import HomeTopbarSubpage from '$lib/components/home/topbar/HomeTopbarSubpage.svelte';
	import type {
		HomeTopbarAction,
		HomeTopbarMetric
	} from '$lib/components/home/topbar/home-topbar.types';
	...
	const shellTitle = $derived(isNotFound ? 'Fallback Route' : 'System Recovery');
	...
	const metrics = $derived<HomeTopbarMetric[]>([
		{ key: 'status', label: 'Status', value: String(status) },
		{ key: 'scope', label: 'Scope', value: isNotFound ? 'Route' : 'System' },
		{ key: 'state', label: 'State', value: 'Fallback' }
	]);
