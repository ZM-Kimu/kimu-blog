<script lang="ts">
	...
	async function rename(item: ManageGroupDocument, event: MouseEvent) {
		event.stopPropagation()
		if (!onrename) return
		const name = window.prompt(renameLabel, item.group.name)?.trim()
		if (!name || name === item.group.name) return
			...
			<div class:active={value === item.group.id} class="manage-select-option-row">
				...
				{#if onrename}
					<button
						...
						data-press-disabled="true"
						onclick={(event) => rename(item, event)}
						type="button"
					>
